...
class BounceYModifier extends Modifier
{
    ...
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
    {
        ...
        noteData.y += currentValue * daswitch * currentValue * NoteMovement.arrowSizes[lane] * Math.abs(FlxMath.fastSin(curPos*0.005*subValues.get('speed').value));
    }
}
...
//LB Shit
class MoveYWaveShitModifier extends Modifier 
{
    override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int){
        noteData.y += 260*Math.sin(((Conductor.songPosition+curPos)*0.0008)+(lane/4))*currentValue;
    }

    override function strumMath(noteData:NotePositionData, lane:Int, pf:Int){
        noteData.y += 260*Math.sin((Conductor.songPosition*0.0008)+(lane/4))*currentValue;
    }
}

class InvertIncomingAngle extends Modifier
{
    override function incomingAngleMath(lane:Int, curPos:Float, pf:Int):Array<Float> {
        if (lane % 2 == 0) {
            return [0, currentValue+(curPos*0.015)];
        }
        return [0, currentValue-(curPos*0.015)];
    }
}
...
// once i figure how to make it work with the other modchart editor ill add a update instantly!
