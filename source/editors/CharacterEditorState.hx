...
class Crosshair extends BitmapData {}
...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		healthBar = new ColorBar(30, FlxG.height - 75);
	...
	function addSettingsUI()
	{
		...
		charDropDown = new FlxUIDropDownMenuCustom(10, 30, FlxUIDropDownMenuCustom.makeStrIdLabelArray([''], true), function(index:String)
		{
	...
	function reloadCharacterImage()
	{
		...
		if(Paths.fileExists('images/' + character.imageFile + '/Animation.json', TEXT))
		{
			...
			{
				trace('Could not load atlas ${character.imageFile}: $e');
			}
	...
	inline function updateHealthBar()
	{
		...
		healthBar.backColorTransform.color = healthBar.frontColorTransform.color = FlxColor.fromRGB(character.healthColorArray[0], character.healthColorArray[1], character.healthColorArray[2]);
	...
	inline function updatePresence() {
		...
		DiscordClient.changePresence("Character Editor", 'Editing ${character.curCharacter}.', "gameicon");
		#end
	}
	...
	function reloadCharacterDropDown() {
		for (file in FileSystem.readDirectory(Paths.getPreloadPath("characters/")))
	...
	function reloadAnimationDropDown() {
		...
		animationDropDown.setData(FlxUIDropDownMenuCustom.makeStrIdLabelArray(animList, true));
	}
