...
class BaseOptionsMenu extends MusicBeatSubstate
{
		...
		for (i in 0...grpOptions.members.length)
		{
			...
			if (grpOptions.members[i].isSelected)
			{
				if (curSelected + 2 < optionsArray.length && curSelected - 2 > 0 && change != 0)
				{
					...
					{
						yLerpTargets[i] = (i
							- (curSelected - (change < 0 ? (curSelected + 2 == optionsArray.length ? 2 : 3) : (curSelected - 2 > 0 ? 3 : 4)))) * 70;
					}
				}
			}
		}
	...
	{
		if (switchStateButtons != null)
			switchStateButtons.clear();
		else
		{
		...
		for (i in 0...options.length)
		{
			if (hideGlasshat && options[i] == 'glasshat')
				continue;
