...
class NewFreeplaySelectSubState extends MusicBeatSubstate
{
	...
	function getMetadatasForCategory()
	{
		...
		var songFolders:Map<String, Bool> = [];

		var dataPath = Paths.getPreloadPath("data/");
		for (assetPath in openfl.utils.Assets.list())
		{
			if (assetPath.startsWith(dataPath))
			{
				var relativePath = StringTools.replace(assetPath, dataPath, "");
				...
				{
					songFolders.set(folderName, true);
				}
			}
		}
		for (folderName in songFolders.keys())
		{
			try
			{
				var metadata:SongMetadata = new SongMetadata(folderName);
			...
			{
				trace('Could not load metadata for folder: $folderName');
			}
		}
	...
	function startGaming()
	{
		...
		FlxTween.tween(bookGroup, {y: bookGroup.y + bookGroup.height}, 0.4, {
	...
	override function update(elapsed:Float)
	{
		if (canInteract)
		{
			...
			else if (leaderboardsButton.isHovered)
			{
				...
				{
					isInOtherSub = true;
					openSubState(new SongLeaderboardSubstate(metadatas[curSelection].displayName, CoolUtil.difficulties[curDifficulty],
			...
			else if (modifiersButton.isHovered)
			{
				...
				{
					isInOtherSub = true;
					openSubState(new FreeplayModifierSelectSubState(metadatas[curSelection].internalName, curDifficulty == 3, chooseSong));
				}
			}
			else
			{
		...
		if (useScrollBar)
		{
			...
			if (this.scrollRect == null)
				this.scrollRect = FlxRect.get(277, 35, 900, 555);
	...
	function makeDifficultyText()
	{
		...
		{
			if (bgHorizontalGroup.members.contains(leftSelector))
				bgHorizontalGroup.remove(leftSelector, true);
			leftSelector.destroy();
			leftSelector = null;
		}
		if (rightSelector != null)
		{
			if (bgHorizontalGroup.members.contains(rightSelector))
				bgHorizontalGroup.remove(rightSelector, true);
		...
		if (availableOptions != null)
		{
			...
			if (bgHorizontalGroup.members.contains(availableOptions))
				bgHorizontalGroup.remove(availableOptions, true);
			availableOptions.destroy();
			availableOptions = null;
		}

		for (file in openfl.utils.Assets.list().filter(path -> path.contains(Paths.getPreloadPath("data/" + metadatas[curSelection].internalName + "/"))))
		{
			//      if(FileSystem.isDirectory(file)) continue;
			if (!file.contains(metadatas[curSelection].internalName))
				continue;

			var diffName = file.replace(metadatas[curSelection].internalName + "-", "").replace(".json", "");
			if (diffName == metadatas[curSelection].internalName)
				diffName = "Normal";
	...
	function makeDifficultyBars()
	{
		if (difficultyBars != null)
		{
			...
			if (bgVerticalGroup.members.contains(difficultyBars))
				bgVerticalGroup.remove(difficultyBars, true);
	...
	function changeSelection(change:Int)
	{
		...
		if (curSelection >= bookSongs.length)
		{
			curSelection = bookSongs.length - 1;
		}
		...
		{
			if (difficultiesAvailable.contains(CoolUtil.defaultDifficulties.indexOf(diff)))
				tempDiff.push(diff);
		}
	...
	function changeDifficulty(change:Int)
	{
		...
		if (curDifficulty >= difficultiesAvailable.length)
		{
			curDifficulty = 0;
		}
		else if (curDifficulty < 0)
		{
			curDifficulty = difficultiesAvailable.length - 1;
		}
		...
		if (clearStar != null)
			bgHorizontalGroup.remove(clearStar);
	...
	function chooseSong()
	{
		...
		if (NewFreeplayState.currentCategory == "freeplay")
		{ // Force normal difficulty on this
...
class BookSong extends FlxSpriteGroup
{
	...
	public function new(songName:String, songFile:String, hasHell:Bool, unlockMethod:String, unlockPrice:Int)
	{
		...
		forEach(function(spr)
		{
			...
			catch (e)
			{
				try
				{
					(cast spr : FlxSpriteGroup).forEach(function(sprr)
					{
						try
						{
							sprr.antialiasing = ClientPrefs.globalAntialiasing;
						}
						catch (eee)
						{
						}
	...
	public function tryUnlockSong(?force:Bool = false)
	{
		...
		{
			if (!force)
				DoorsUtil.addKnobs(-unlockPrice, 1.0);
...
class Star extends FlxSprite
{
	...
	function getStarType():StarType
	{
		...
		{
			if (isHell && i != CoolUtil.defaultDifficulties.indexOf("Hell"))
				continue;
			else if (!isHell && i == CoolUtil.defaultDifficulties.indexOf("Hell"))
				continue;
			...
			if (Highscore.getMisses(attachedMetadata.internalName + (isHell ? "-hell" : "")) >= 0
				|| Highscore.getRating(attachedMetadata.internalName, i) != 0)
			{
